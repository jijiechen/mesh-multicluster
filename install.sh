...
for DATA_PLANE in "${DATA_PLAINE_NAMES[@]}" ; do
    sed "s/ISTIOD_REMOTE_EP/$ISTIOD_REMOTE_EP/" ./data-plane.yaml | sed "s/DATA_PLANE_NAME/$DATA_PLANE/" > ./.install/data-plne-$DATA_PLANE-values.yaml

    istioctl manifest generate -f ./.install/data-plne-$DATA_PLANE.yaml -n $ISTIO_NAMESPACE > ./.install/data-plne-$DATA_PLANE-install.yaml
    
    kubectl apply -f ./.install/data-plne-$DATA_PLANE-install.yaml  -n $ISTIO_NAMESPACE --context=ctx-${DATA_PLANE} 
